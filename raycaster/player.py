...
class Player:
    ...
    def __rotate(self, rel_mouse_motion):
    ...
    def move(self, keyboard: Keyboard, delta_time):
        if keyboard.W:
            self.__move_y(delta_time)
        if keyboard.S:
            self.__move_y(-delta_time)

        if keyboard.D:
            self.__move_x(delta_time)
        if keyboard.A:
            self.__move_x(-delta_time)

        if keyboard.MOUSE_MOTION:
            self.__rotate(keyboard.MOUSE_MOTION * delta_time)
